import puppeteer from "puppeteer";

const style = `<style>
* {
  font-family: verdana;
  font-size: 10px;
}
 
table, th, td {
  border: 1px solid darkgray;
  border-collapse: collapse;
  margin-bottom: 40px;
  padding: 5px;
  word-break: break-all;

}
 
.principal {
  display: flex;
  justify-content: center
}
 
.header {
  width: 120px;
  height: 40px;
  text-align: left;
}
 
.p-sp {
  margin-left: 540px;
  margin-bottom: 40px
}
 
h3 {
  font-family: verdana;
  text-align: center;
  margin-bottom: 30px;
}
 
.td-special {
  width: 20px;
}
 
.tabla-firma {
  margin-top: 50px;
  display: flex;
  justify-content: center
}
 
.gris {
  background-color: lightgray;
}
 
.cuadrado {
  width: 350px;
  height: 150px;
  background: white;
  border: 2px solid darkgray;
  margin: 0 auto;
  font-weight: 800;
  padding: 5px;
}
 
.cobertura-seguro{
  margin-top: 80px;
  margin-left: 100px;
  margin-right: 100px;
  font-size: 18px;
}
 
li {
  padding: 10px;
  font-size: 11px;
  text-align: justify;
}
 
.imagenes {
  text-align: center;
  padding: 10px;
}
 
.page-break {
  page-break-before: always; /* Forzar un salto de página antes de este elemento */
}
</style>`;

const fnApitest = async (data) => {
  const htmlContent = `
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <link rel="stylesheet" href="css.css">
    ${style}
</head>
<body>
  <div class="header">
    <img src="data:image/svg+xml;base64,">
  </div>
  <p class="p-sp"><b> N° pedido asociado:</b> __________</p>
  <h3>ACTA ENTREGA / RECEPCIÓN DE VEHÍCULOS N°: __________</h3>
  <div class="principal">
    <table>
      <tr>
        <td><b>Preparado por: </b>${data.DATOS_INSPECCION.preparado_por}</td>
        <td><b>Fecha: </b>${data.DATOS_INSPECCION.fecha_inspeccion}</td>
        <td><b>Hora Recepción: </b> / <b>Hora Inspección: </b>${
          data.DATOS_INSPECCION.hora_recepcion
        }</td>
      </tr>
      <tr>
        <td><b>Patente: </b>${data.DATOS_INSPECCION.patente_vehiculo}</td>
        <td><b>Marca: </b>${data.DATOS_INSPECCION.marca_vehiculo}</td>
        <td><b>Modelo: </b>${data.DATOS_INSPECCION.modelo_vehiculo}</td>
      </tr>
      <tr>
        <td><b>Chasis: </b>${data.DATOS_INSPECCION.chasis_vehiculo}</td>
        <td><b>Color: </b>${data.DATOS_INSPECCION.color_vehiculo}</td>
        <td><b>Año: </b>${data.DATOS_INSPECCION.anio_vehiculo}</td>
      </tr>
      <tr>
        <td colspan=3><b>Cliente: </b>${
          data.DATOS_INSPECCION.preparado_por
        }</td>
      </tr>
      <tr>
        <td><b>Kilometraje: </b>${data.DATOS_INSPECCION.preparado_por}</td>
        <td><b>Combustible: </b>${data.DATOS_INSPECCION.preparado_por}</td>
        <td><b>Tipo de Servicio: </b>${data.DATOS_INSPECCION.preparado_por}</td>
      </tr>
      <tr>
        <td><b>Nombre Chofer: </b>${data.DATOS_INSPECCION.preparado_por}</td>
        <td><b>Celular: </b>${data.DATOS_INSPECCION.preparado_por}</td>
        <td><b>Mail: </b>${data.DATOS_INSPECCION.preparado_por}</td>
      </tr>
      <tr>
        <td colspan=3><b>Observaciones: ${
          data.DATOS_INSPECCION.observaciones_inspeccion
        }</b> </td>
      </tr>
    </table>
  </div>
 
 
  <div class="principal">
    <table>
      <thead class="gris">
        <tr>
          <th>Nombre del Artículo</th>
          <th>Existe</th>
          <th>Estado</th>
          <th>Comentario</th>
        </tr>
      </thead>
      <tbody>
        ${data.ACCESORIOS_INSPECCION.map(
          (articulo) => `
        <tr>
          <td>${articulo.nombreArticulo}</td>
          <td>${articulo.Existe}</td>
          <td>${articulo.Estado}</td>
          <td>${articulo.Comentario}</td>
        </tr>
        `
        ).join("")}
      </tbody>
    </table>
  </div>
 
 
  <div class="page-break";>
    <div class="header">
      <img src="data:image/svg+xml;base64,">
    </div>
 
    <div class="tabla-firma">
      <table>
        <tr>
          <th>POR GAMALEASING SPA</th>
          <th>USUARIO CLIENTE</th>
        </tr>
        <tr>
          <td><b>NOMBRE:</b> ${data.DATOS_INSPECCION.preparado_por}</td>
          <td><b>NOMBRE:</b> ${data.DATOS_INSPECCION.preparado_por}</td>
        </tr>
        <tr>
          <td><b>RUT:</b> ${data.DATOS_INSPECCION.preparado_por}</td>
          <td><b>RUT:</b> ${data.DATOS_INSPECCION.preparado_por}</td>
        </tr>
      </table>
    </div>
  </div>
 
    <div class="cuadrado">
      FIRMA CLIENTE:
    </div>
 
    <div class="cobertura-seguro">
      <ol>
        <li>
          Cobertura del Seguro se hará efectiva solamente si se realiza la constancia policial inmediatamente ocurrido el Siniestro.
        </li>
        <li>
          Todo vehículo de reemplazo considera depósito de combustible lleno en su entrega, el cual debe retornar de igual manera,
          en caso contrario la diferencia será facturado al cliente.
        </li>
      </ol>
    </div>
 
    <div class="imagenes">
      ${data.FOTOS_INSPECCION.map(
        (imagen) => `
        <div class="page-break";>
          <div class="header">
            <img src="data:image/svg+xml;base64,">
          </div>
          <h5>${imagen.nombre_foto}</h5>
            <img src="${("data:image/png;base64,", imagen.$content)}">
              <p><b>Comentario: </b>${imagen.comentario_foto}</p>
        </div>
      `
      ).join("")}
    </div>
 
</body>
</html>
  `;

  async function generatePDF() {
    const browser = await puppeteer.launch();
    const page = await browser.newPage();

    // Cargar el HTML dinámico en la página
    await page.setContent(htmlContent);

    // Configurar las opciones del PDF
    await page.pdf({
      path: "dynamic_images.pdf",
      format: "LETTER",
      printBackground: true,
      margin: {
        top: "10px",
        bottom: "10px",
        left: "10px",
        right: "10px",
      },
    });
    await page.word;
    await browser.close();
  }
  generatePDF();
};

export default fnApitest;
